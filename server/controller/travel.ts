...
export const getTravels = async (
    ...
    try {
        ...
        return res.status(200).json({ success: true, data: travels });
    } catch (error: any) {
        return res.status(500).json({
            success: false,
            message:
...
export const getTravelTravellers = async (
    req: Request,
    res: Response
): Promise<Response> => {
    try {
        const { id } = req.params;
        const travel = await travelModel.selectTravelWithTravellers(Number(id));
        ...
        return res.status(500).json({
            ...
            message:
                error.message ||
                'An error occurred while fetching travel travellers.',
        });
    }
};
